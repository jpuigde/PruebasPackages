##
 ...
 #------------------------------------------------------
                        #-------------------------------
                         ...
                         df1<-m[1:9,1:2]
                         names(df1)<-c("order","rgb")
                         #------------------------------
 library( reshape2 )
 #ls( "package:circlize" )
 
 setwd( "C:/Proyectos/MRW/01-data" )
 #list.files( )
 
 RawRow <- read.csv("Row ordestbult.csv",sep = ";",stringsAsFactors=FALSE)
 colnames(RawRow)
 
 
 m<-dcast(RawRow,nombreorigen~nombredestino)
 ...
 dimnames(m)<-list(orig=Rownames, dest=ColNames[2:length(ColNames)])
